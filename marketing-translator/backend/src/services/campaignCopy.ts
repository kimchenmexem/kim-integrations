...
function buildBatchSystemPrompt(
  ...
  return [
    ...
    `Output every text field directly in ${language}. Do not output English unless the target language IS English.`,
    ``,
    `BRIEF ANCHORING (CRITICAL — overrides every style rule below):`,
    `The "Marketing message" in the user prompt is the CANONICAL claim for this campaign. Every headline and subheadline you write MUST be a faithful restatement, narrowing, or natural extension of something that is ALREADY in the marketing_message.`,
    `- Use the same products, services, and properties the marketing_message names.`,
    `- Do NOT introduce claims (numbers, percentages, "zero commission", "commission-free", "free", "guaranteed", "best-in-class", specific market counts, product features) that are not present in the marketing_message verbatim.`,
    `- "transparent costs" does NOT permit "zero commission". "global investment products" does NOT permit "170+ markets". "tools designed for investors" does NOT permit "AI-powered tools". When in doubt, paraphrase what the brief literally says rather than expanding it.`,
    `- Each concept's strategic_idea narrows the ANGLE on the brief (which facet to lead with) — it cannot introduce new product claims. If a concept's strategic_idea appears to contradict or exceed the marketing_message, defer to the marketing_message wording.`,
    `- A reader should be able to point at every headline and identify the phrase from the marketing_message it's restating.`,
    ``,
    `Tone must be professional, factual, and compliant with EU financial-marketing regulation (ESMA / national regulator equivalents).`,
    `Style: calm, concrete, platform-led. Prefer specific product/value language ("market access", "tools", "data", "platform") over campaign slogans.`,
    `Avoid: guarantees of return, urgency tactics, superlative claims ("best", "leader"), oversimplification ("easy", "effortless"), and aggressive trading language ("command", "dominate", "win", "stay ahead", "beat the market", "unlock").`,
